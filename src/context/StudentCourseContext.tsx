  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center w-full h-full min-h-[50vh] space-y-4">
        ...
        <p className="text-muted-foreground font-medium animate-pulse">Đang tải không gian khóa học...</p>
      </div>
    );
  }
  ...
  if (error || !course) {
    return (
      <div className="flex flex-col items-center justify-center w-full h-full min-h-[50vh] space-y-4 p-8 text-center">
        ...
        <p className="text-muted-foreground max-w-md">Khóa học không tồn tại hoặc bạn không có quyền truy cập vào không gian này.</p>
      </div>
    );
  }
...
export function useStudentCourse() {
  ...
  if (context === undefined) {
    throw new Error("useStudentCourse must be used within a StudentCourseProvider");
  }
  return context;
}
