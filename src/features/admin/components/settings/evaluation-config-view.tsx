    ...
    <div className="space-y-6">
      <Tabs defaultValue="ai-warnings" value={activeTab} onValueChange={setActiveTab} className="w-full">
        <TabsList className="mb-6 h-auto rounded-2xl bg-muted/50 border border-border/50 p-1.5 gap-2">
          <TabsTrigger value="ai-warnings" className="rounded-xl px-6 py-2.5 font-medium data-[state=active]:bg-background data-[state=active]:shadow-sm flex items-center justify-center gap-2">
            <AlertTriangle className="w-4 h-4" /> Ngưỡng Cảnh báo AI
          </TabsTrigger>
          <TabsTrigger value="requests" className="relative rounded-xl px-6 py-2.5 font-medium data-[state=active]:bg-background data-[state=active]:shadow-sm flex items-center justify-center gap-2">
