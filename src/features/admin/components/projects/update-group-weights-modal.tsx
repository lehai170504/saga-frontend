...
import { useUpdateGroupWeights } from "../../hooks/useProjects";
import { toast } from "sonner";
import { ProjectDetailResponse } from "../../api/projectApi";

interface UpdateGroupWeightsModalProps {
  project: ProjectDetailResponse;
  trigger?: React.ReactNode;
}

export function UpdateGroupWeightsModal({ project, trigger }: UpdateGroupWeightsModalProps) {
  ...
  const updateWeights = useUpdateGroupWeights();
    ...
    <Dialog open={open} onOpenChange={handleOpenChange}>
      ...
      <DialogContent className="sm:max-w-[425px] rounded-3xl bg-background/95 backdrop-blur-3xl border-border/50">
        <DialogHeader>
          ...
          <DialogDescription>
            Điều chỉnh trọng số đánh giá riêng cho nhóm <strong className="text-foreground">{project.team.teamName}</strong>.
          </DialogDescription>
        </DialogHeader>
