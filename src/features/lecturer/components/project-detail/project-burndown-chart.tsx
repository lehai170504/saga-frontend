...
export function ProjectBurndownChart({ courseId, teamId }: { courseId: string; teamId: string }) {
  ...
  useEffect(() => {
    if (velocityData?.sprints && velocityData.sprints.length > 0 && !selectedSprintId) {
      // Pick the first sprint as default (or active)
      setSelectedSprintId(velocityData.sprints[0].sprintId);
    }
  }, [velocityData, selectedSprintId]);

  const { data: burndownData, isLoading: isLoadingBurndown } = useSprintBurndown(courseId, teamId, selectedSprintId);

  const isLoading = isLoadingSprints || isLoadingBurndown;

  const chartData = React.useMemo(() => {
    if (!burndownData?.points) return [];
    return burndownData.points.map((p: any) => ({
  ...
  const currentPoint = React.useMemo(() => {
    ...
    const pastOrToday = burndownData.points.filter((p: any) => p.date <= today);
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        {velocityData?.sprints && velocityData.sprints.length > 0 && (
          <Select value={selectedSprintId} onValueChange={setSelectedSprintId} disabled={isLoading}>
