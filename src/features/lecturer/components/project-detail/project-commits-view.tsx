...
export function ProjectCommitsView({ projectId, repositories = [] }: ProjectCommitsViewProps) {
  ...
  const isLoading = isLoadingBranches || (!!selectedBranch && isLoadingCommits);
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-card/40 border border-border/50 backdrop-blur-xl p-4 rounded-2xl">
            ...
            <div className="flex flex-col items-start gap-2.5 w-full sm:w-auto sm:max-w-[50%] shrink-0">
              ...
              <div className="w-full">
                ...
                >
                  <SelectTrigger className="rounded-xl border-border bg-background/50 h-10 font-medium text-xs flex items-center gap-2 w-full [&>span]:truncate">
          ...
          {isLoading ? (
            ...
            <Card className="rounded-[2rem] border border-border/50 bg-card/25 backdrop-blur-xl p-8 text-center space-y-4">
              ...
              <p className="text-muted-foreground text-xs max-w-sm mx-auto">
                Hiện chưa có commit nào được đồng bộ từ kho lưu trữ hoặc nhánh này.
              </p>
            </Card>
          ) : (
            <div className="space-y-4">
              <div className="flex items-center justify-between border-b border-border/40 pb-2">
                <h4 className="font-extrabold text-xs uppercase tracking-wider text-muted-foreground">
                  Danh sách Commit ({selectedRepo?.repositoryName || commitData?.repositoryName || "GitHub"})
                </h4>
